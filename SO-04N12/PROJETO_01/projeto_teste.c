...
int main()
{
    ...
    printf("Digite o valor na conta from: ");
    scanf("%d", &from.saldo);
    printf("Digite o valor na conta to: ");
    scanf("%d", &to.saldo);
    
    int loop = from.saldo/10;
    ...
    for (i = 0; i < loop; i++) {
        ...
        SIGCHLD | CLONE_FS | CLONE_FILES | CLONE_SIGHAND | CLONE_VM | CLONE_VFORK, 0);
        if (pid == -1) {
            perror("clone");
            exit(2);
        }
	...
	/*for (i = 0; i < 0; i++) {
        ...
        pid = clone(&transferencia2, (char*) stack + FIBER_STACK,
        SIGCHLD | CLONE_FS | CLONE_FILES | CLONE_SIGHAND | CLONE_VM | CLONE_VFORK, 0);
        if (pid == -1){
            perror("clone");
            exit(2);
